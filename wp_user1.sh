...
if [ -f "$input" ]; then
    ...
    while IFS= read -r site; do
        ...
        echo -e "#############################################################" | tee -a deletethis
 #       echo -e "${CYAN}Enumerating users at $input ...${NORMAL}"
        echo -e "${CYAN}Enumerating users at $url ...${NORMAL}" | tee -a deletethis
        response=$(curl -sL "$url")

        # Check if the answer is empty
        if [ -z "$response" ]; then
            echo -e "${RED}No response received for $site. \
                  The site might be blocking the request or the URL is incorrect.${NORMAL}" | tee -a deletethis
            continue
        fi

        # 'name' ve 'slug' find and display values ​of tags
        echo -e "${GREEN}Found users for $site:${NORMAL}" | tee -a deletethis
        echo "$response" | grep -oP '"name":"[^"]*"|"slug":"[^"]*"' | \
                sed -E 'N;s/"name":"([^"]*)".*"slug":"([^"]*)"/Name: \1 - Username: \2/' | \
                while read -r user; do
            echo -e "${GREEN}- ${user}${NORMAL}" | tee -a deletethis
        done
    ...
    # Check if the answer is empty
    if [ -z "$response" ]; then
        echo -e "${RED}No response received. The site might be blocking the request \
            or the URL is incorrect.${NORMAL}" | tee -a deletethis
        exit 1
    fi

    # 'name' ve 'slug' find and display values ​​of tags
    echo -e "${GREEN}Found users:${NORMAL}" | tee -a deletethis
     echo "$response" | grep -oP '"name":"[^"]*"|"slug":"[^"]*"' | \
          sed -E 'N;s/"name":"([^"]*)".*"slug":"([^"]*)"/Name: \1 - Username: \2/' | \
          while read -r user; do
        echo -e "${GREEN}- ${user}${NORMAL}" | tee -a deletethis
